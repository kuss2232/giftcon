		...
		<div id="page-wrapper">
				...
				<table class="board_view">
					...
					<tbody>
						<tr>
							...
							<td colspan="3">${goodsBasic.GOODS_NUM}<input type="hidden"
								id="GOODS_NUM" name="GOODS_NUM" class="wdp_90"
								value="${goodsBasic.GOODS_NUM}"></input></td>
						</tr>
						<tr>
							<th scope="row">상품명</th>
							<td><input type="text" id="GOODS_NAME" name="GOODS_NAME"
								class="wdp_90" value="${goodsBasic.GOODS_NAME}"></input></td>
						</tr>

						<tr>
							<th scope="row">카테고리</th>
							<td><select id="BIG_CATEGORY" name="BIG_CATEGORY"
								onchange="BIG_CATEGORYChange();" size=1>
							...
							</select> <SELECT id="SMALL_CATEGORY" name="SMALL_CATEGORY" size=1>
						...
						<tr>
							...
							<td><input type="text" id="GOODS_PRICE" name="GOODS_PRICE"
								class="wdp_90" value="${goodsBasic.GOODS_PRICE}"></input></td>
						</tr>
						<tr>
							<th scope="row">할인 가격</th>
							<td><input type="text" id="GOODS_DCPRICE"
								name="GOODS_DCPRICE" class="wdp_90"
								value="${goodsBasic.GOODS_DCPRICE}"></input></td>
						</tr>
						<tr>
							<td colspan="2" class="view_text"><textarea rows="5"
									cols="100" title="내용" id="GOODS_SIMPLE" name="GOODS_SIMPLE"
									placeholder="기본 설명">${goodsBasic.GOODS_SIMPLE}</textarea> <br />
								<br /> <textarea rows="10" cols="100" title="내용"
									id="GOODS_DETAIL" name="GOODS_DETAIL" placeholder="상세 설명">${goodsBasic.GOODS_DETAIL}</textarea>
								<br /> <br /></td>
						</tr>
						<tr>
							<th scope="row">물량 수정</th>
							<td><input type="text" id="GOODS_AMOUNT" name="GOODS_AMOUNT"
								class="wdp_90" value="${goodsBasic.GOODS_AMOUNT}"></input></td>
							<c:if test="${GOODS_AMOUNT == 0}">
								<input type="hidden" id="GOODS_SOLDOUT" name="GOODS_SOLDOUT"
									value="Y">
							</c:if>
							<c:if test="${GOODS_AMOUNT != 0}">
								<input type="hidden" id="GOODS_SOLDOUT" name="GOODS_SOLDOUT"
						...
						<tr>
							...
							<td colspan="3"><input type="hidden" name="ORIGINAL_IMG"
								value="${goodsBasic.GOODS_IMG}">
								<p>
									<c:set var="i" value="0" />
									<c:set var="logo" value="${fn:split(goodsBasic.GOODS_IMG,',')}" />
									<c:forEach var="logo" items="${logo}">
										<c:if test="${i eq 0}">
											<img src="/giftcon/resources/file/goodsFile/${logo}"
												width="60" height="60" alt="">
											<c:set var="i" value="1" />
										</c:if>
									</c:forEach>
									<br /> <input type="file" id="file"
										name="MODIFY_IMG_${goodsBasic.GOODS_IMG}"> <input
										type="hidden" name="${goodsBasic.GOODS_IMG}" value="1">
								</p></td>
						</tr>
					</tbody>
				</table>

				<br /> <br /> <a href="#this" class="btn" id="write">작성하기</a> <a
			...
			<script type="text/javascript">
				...
				function fn_openGoodsList() {
					...
					comSubmit
							.setUrl("<c:url value='/goods/adminGoodsList.conn' />");
					comSubmit.submit();
				}
				...
				function fn_insertGood() {
					...
					comSubmit
							.setUrl("<c:url value='/goods/adminModifyGoods.conn' />");
					comSubmit.submit();
				}
			</script>
