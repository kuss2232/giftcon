...
public class AdminQNAPaging {
	...
	//검색할 경우 페이징 생성자
	public AdminQNAPaging(int currentPage, int totalCount, int blockCount,int blockPage, String list, int searchNum, String SearchKeyword) {
		...
		if(currentPage > blockPage) {
			if(SearchKeyword == null) {
				...
				pagingHtml.append("<a class='page prv' href=" + list + "?&searchNum=" + searchNum + "&SearchKeyword=" + SearchKeyword + "&currentPage=" + (startPage -1) +">");
			}
			pagingHtml.append("&lt");
			pagingHtml.append("</a>");	
		}
		...
		for(int i= startPage; i<= endPage; i++) {
			...
			if(i == currentPage) {
				...
				if(SearchKeyword == null) {
					pagingHtml.append("<a class='page' href=" + list + "&currnetPage=");
				} else {
					pagingHtml.append("<a class='page' href=" + list + "?&searchNum=" + searchNum + "&SearchKeyword=" + SearchKeyword + "&currentPage=");
				}
				pagingHtml.append(i);
				pagingHtml.append(">");
				pagingHtml.append(i);
		...
		if(totalPage - startPage >= blockPage) {
			if(SearchKeyword == null) {
				...
				pagingHtml.append("<a class='page next' href=" + list + "?searchNum=" + searchNum + "&SearchKeyword=" + SearchKeyword + "&currentPage=" + (endPage +1) +">");
			}
			pagingHtml.append("&gt;");
			pagingHtml.append("</a>");
		}
	...
	public AdminQNAPaging(int currentPage, int totalCount, int blockCount, int blockPage, String list) {
		...
		public AdminQNAPaging(int currentPage, int totalCount, int blockCount, int blockPage, String list, Integer categoryNum, HttpServletRequest request) {
			...
			//마지막 페이지가 전체페이지 수보다 크면 전체 페이지 수로 설정
			if(endPage > totalPage) {
				endPage = totalPage;
			}
